import modules.subscene as subscene
from entities.movie import Movie
import sys, os, time, atexit, signal

#language = 'English'
# language = input("Enter your preferenced Language:")
# pathToDirectory = input("Enter movie path: ")
#r'C:\Users\hemanta.devi.huril\Desktop\movies'

def main(language, pathToDirectory):
    # Get all movies from folder
    movies = subscene.getListOfMovies(pathToDirectory)
    #print(movies)
    
    for movie in movies:
        print(movie.getMovieName())
        # Get movie page
        webpage = subscene.getMoviePage(movie, language)
        #print("Webpage", webpage)
        
        # Get all links
        html_page = subscene.getHtmlPage(webpage, movie, language)
        #print("Links: ", html_page)
        
        if html_page:
            #Activate a download link
            secondWebPage = subscene.getWebpageFromFirstDownloadLink(html_page)
            #print("second webpage: ",secondWebPage)
            
            #Get download link from second webpage
            confirm_link = subscene.downloadSubtitleForMovie(secondWebPage)
            #print(confirm_link)
            
            #Download subtitles
            subscene.activateDownloadLink(confirm_link, movie.getFolder(), pathToDirectory)
        else:
            #if page was not obtained
            print("Page was not obtained")
...
if __name__ == '__main__':
    ...
    dc = daemon_ctl(Daemon, pidfile, os.getcwd())
